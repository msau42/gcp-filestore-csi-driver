...
func RunRemote(suite TestSuite, archive string, host string, cleanup bool, testArgs string, ginkgoArgs string) (string, bool, error) {
	// Install and start NFS dependencies
	if output, err := SSH(host, "apt-get", "install", "-y", "nfs-common"); err != nil {
		return "", false, fmt.Errorf("failed to install nfs client on host %q: %v output: %q", host, err, output)
	}
