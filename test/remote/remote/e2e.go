...
// E2ERemote is type for Google Cloud Filestore CSI Driver Remote E2E Tests
type E2ERemote struct{}

// InitE2ERemote initializes the Google Cloud Filestore CSI Driver remote E2E suite
...
func (n *E2ERemote) SetupTestPackage(tardir string) error {
	...
	out, err := exec.Command("go", "test", "-c", "sigs.k8s.io/gcp-filestore-csi-driver/test/e2e", "-o", "test/e2e/e2e.test").CombinedOutput()
	if err != nil {
		return fmt.Errorf("Failed to build test: %v\nOutput:\n%s", err, out)
	}

	out, err = exec.Command("mkdir", "-p", "bin").CombinedOutput()
	if err != nil {
		return fmt.Errorf("Failed to mkdir bin/: %v\nOutput:\n%s", err, out)
	}

	out, err = exec.Command("cp", "test/e2e/e2e.test", "bin/e2e.test").CombinedOutput()
	if err != nil {
